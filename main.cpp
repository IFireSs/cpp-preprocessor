...
bool Preprocess(const path& in_file, ofstream& output, const vector<path>& include_directories) {
    if (ifstream i_stream = ifstream(in_file)) {
        ...
        for (int num_of_cur_str = 1; getline(i_stream, cur_str); ++num_of_cur_str) {
            if (regex_match(cur_str, match, QUOTATION_MARKS)) {
                if (!Preprocess(in_file.parent_path() / string(match[1]), output, include_directories) && !CheckDirectories(in_file, output, include_directories, string(match[1]), num_of_cur_str)) {
                    return false;
                }
            }
            else if (regex_match(cur_str, match, SANGLE_BRACKETS)) {
                if (!CheckDirectories(in_file, output, include_directories, string(match[1]), num_of_cur_str)) {
                    return false;
                }
            }
            else {
